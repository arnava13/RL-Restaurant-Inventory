# -*- coding: utf-8 -*-
"""Project_Env+Demand.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15nigNiprXZRWhXrxQa3wLR_ZxWdyZ79t
"""
...
class KitchenEnv(gym.Env):
    ...
    def _init_gp(self):
        ...
        for j in range(self.n_prep):
            # Kernel per dish
            kernel = C(1.0, (0.01, 20)) * RBF(length_scale=5.0)
            ...
            # Initialize GP with a single starting point
            X0 = np.array([[0]])
    ...
    def render(self, mode="human"):
        max_raw = int(max(self.raw_shelf_life))
        max_prep = int(max(self.prep_shelf_life))

        raw_mat = np.zeros((self.n_raw, max_raw))
        prep_mat = np.zeros((self.n_prep, max_prep))

        for i in range(self.n_raw):
            raw_mat[i, : len(self.raw_inv[i])] = self.raw_inv[i]
        for j in range(self.n_prep):
            prep_mat[j, : len(self.prep_inv[j])] = self.prep_inv[j]

        plt.figure(figsize=(8, 4))
        plt.subplot(1, 2, 1)
        plt.imshow(raw_mat, cmap="YlGn", aspect="auto", origin="lower")
        plt.title(f"Raw Inventory (Day {self.day})")
        plt.colorbar()
...
def plot_inventory(env):
    """
    Plots total stock for each raw ingredient and prepared dish.
    """
    # --- Raw ingredient totals ---
    raw_totals = [inv.sum() for inv in env.raw_inv]
    raw_labels = [ing.name for ing in env.ingredients]

    # --- Prepared dish totals ---
    prep_totals = [inv.sum() for inv in env.prep_inv]
    prep_labels = [d.name for d in env.dishes]
    ...
    # Raw inventory
    plt.subplot(1,2,1)
    plt.bar(raw_labels, raw_totals, color="skyblue")
    plt.title("Raw Ingredient Inventory")
    plt.ylabel("Units in Stock")
    plt.xlabel("Ingredient")

    # Prepared inventory
    plt.subplot(1,2,2)
    plt.bar(prep_labels, prep_totals, color="salmon")
    plt.title("Prepared Dish Inventory")
    plt.ylabel("Units in Stock")
    plt.xlabel("Dish")
...
for past, gp in zip([past0, past1], gp_models):
    ...
    for t in days:
        ...
        f = np.random.normal(mean[0], min(std[0], 0.2))
        lam = np.exp(f)
        lam = min(lam, 80)
